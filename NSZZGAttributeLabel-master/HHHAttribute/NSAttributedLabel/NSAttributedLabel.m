...
#define kNSLineBreakWordWrapTextWidthScalingFactor (M_PI / M_E)

static CGFloat const NSFLOAT_MAX = 100000;

NSString * const kNSStrikeOutAttributeName = @"NSStrikeOutAttribute";
NSString * const kNSBackgroundFillColorAttributeName = @"NSBackgroundFillColor";
NSString * const kNSBackgroundFillPaddingAttributeName = @"NSBackgroundFillPadding";
NSString * const kNSBackgroundStrokeColorAttributeName = @"NSBackgroundStrokeColor";
NSString * const kNSBackgroundLineWidthAttributeName = @"NSBackgroundLineWidth";
NSString * const kNSBackgroundCornerRadiusAttributeName = @"NSBackgroundCornerRadius";

const NSTextAlignment NSSTextAlignmentLeft = NSTextAlignmentLeft;
const NSTextAlignment NSSTextAlignmentCenter = NSTextAlignmentCenter;
const NSTextAlignment NSSTextAlignmentRight = NSTextAlignmentRight;
const NSTextAlignment NSSTextAlignmentJustified = NSTextAlignmentJustified;
const NSTextAlignment NSSTextAlignmentNatural = NSTextAlignmentNatural;

const NSLineBreakMode NSSLineBreakByWordWrapping = NSLineBreakByWordWrapping;
const NSLineBreakMode NSSLineBreakByCharWrapping = NSLineBreakByCharWrapping;
const NSLineBreakMode NSSLineBreakByClipping = NSLineBreakByClipping;
const NSLineBreakMode NSSLineBreakByTruncatingHead = NSLineBreakByTruncatingHead;
const NSLineBreakMode NSSLineBreakByTruncatingMiddle = NSLineBreakByTruncatingMiddle;
const NSLineBreakMode NSSLineBreakByTruncatingTail = NSLineBreakByTruncatingTail;

typedef NSTextAlignment NSTextAlignment;
typedef NSLineBreakMode NSLineBreakMode;
...
static inline CGFloat NSFlushFactorForTextAlignment(NSTextAlignment textAlignment) {
    switch (textAlignment) {
        case NSSTextAlignmentCenter:
            return 0.5f;
        case NSSTextAlignmentRight:
            return 1.0f;
        case NSSTextAlignmentLeft:
        default:
            return 0.0f;
    }
}
...
static inline NSDictionary * NSAttributedStringAttributesFromLabel(NSAttributedLabel *label) {
    ...
    if (label.numberOfLines == 1) {
        ...
        paragraphStyle.lineBreakMode = NSSLineBreakByWordWrapping;
    }
...
static inline CGSize CTFramesetterSuggestFrameSizeForAttributedStringWithConstraints(CTFramesetterRef framesetter, NSAttributedString *attributedString, CGSize size, NSUInteger numberOfLines) {
    ...
    CGSize constraints = CGSizeMake(size.width, NSFLOAT_MAX);

    if (numberOfLines == 1) {
        // If there is one line, the size that fits is the full width of the line
        constraints = CGSizeMake(NSFLOAT_MAX, NSFLOAT_MAX);
    } else if (numberOfLines > 0) {
        // If the line count of the label more than 1, limit the range to size to the number of lines that have been set
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathAddRect(path, NULL, CGRectMake(0.0f, 0.0f, constraints.width, NSFLOAT_MAX));
...
- (NSAttributedLabelLink *)linkAtPoint:(CGPoint)point {
    ...
    NSAttributedLabelLink *result = [self linkAtCharacterIndex:[self characterIndexAtPoint:point]];
...
- (NSAttributedLabelLink *)linkAtRadius:(const CGFloat)radius aroundPoint:(CGPoint)point {
    ...
    NSAttributedLabelLink *link = nil;
...
- (NSAttributedLabelLink *)linkAtCharacterIndex:(CFIndex)idx {
                ...
                context:(CGContextRef)c
{
    ...
    CFArrayRef lines = CTFrameGetLines(frame);
    NSInteger numberOfLines = self.numberOfLines > 0 ? MIN(self.numberOfLines, CFArrayGetCount(lines)) : CFArrayGetCount(lines);
    BOOL truncateLastLine = (self.lineBreakMode == NSSLineBreakByTruncatingHead || self.lineBreakMode == NSSLineBreakByTruncatingMiddle || self.lineBreakMode == NSSLineBreakByTruncatingTail);
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        if (lineIndex == numberOfLines - 1 && truncateLastLine) {
            ...
            if (!(lastLineRange.length == 0 && lastLineRange.location == 0) && lastLineRange.location + lastLineRange.length < textRange.location + textRange.length) {
                ...
                if (numberOfLines != 1) {
                    lineBreakMode = NSSLineBreakByTruncatingTail;
                }

                switch (lineBreakMode) {
                    case NSSLineBreakByTruncatingHead:
                        truncationType = kCTLineTruncationStart;
                        break;
                    case NSSLineBreakByTruncatingMiddle:
                        truncationType = kCTLineTruncationMiddle;
                        truncationAttributePosition += (lastLineRange.length / 2);
                        break;
                    case NSSLineBreakByTruncatingTail:
                    default:
               ...
               context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            CGColorRef strokeColor = CGColorRefFromColor([attributes objectForKey:kNSBackgroundStrokeColorAttributeName]);
            CGColorRef fillColor = CGColorRefFromColor([attributes objectForKey:kNSBackgroundFillColorAttributeName]);
            UIEdgeInsets fillPadding = [[attributes objectForKey:kNSBackgroundFillPaddingAttributeName] UIEdgeInsetsValue];
            CGFloat cornerRadius = [[attributes objectForKey:kNSBackgroundCornerRadiusAttributeName] floatValue];
            CGFloat lineWidth = [[attributes objectForKey:kNSBackgroundLineWidthAttributeName] floatValue];
           ...
           context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            NSDictionary *attributes = (__bridge NSDictionary *)CTRunGetAttributes((__bridge CTRunRef) glyphRun);
            BOOL strikeOut = [[attributes objectForKey:kNSStrikeOutAttributeName] boolValue];
...
- (void)setActiveLink:(NSAttributedLabelLink *)activeLink {
     ...
     limitedToNumberOfLines:(NSInteger)numberOfLines
{
    ...
    if (textSize.height < bounds.size.height) {
        ...
        switch (self.verticalAlignment) {
            case NSAttributedLabelVerticalAlignmentCenter:
                yOffset = CGFloat_floor((bounds.size.height - textSize.height) / 2.0f);
                break;
            case NSAttributedLabelVerticalAlignmentBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
            case NSAttributedLabelVerticalAlignmentTop:
            default:
                break;
        }
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.adjustsFontSizeToFitWidth && self.numberOfLines > 0) {
        ...
        CGSize maxSize = (self.numberOfLines > 1) ? CGSizeMake(NSFLOAT_MAX, NSFLOAT_MAX) : CGSizeZero;

        CGFloat textWidth = [self sizeThatFits:maxSize].width;
        CGFloat availableWidth = self.frame.size.width * self.numberOfLines;
        if (self.numberOfLines > 1 && self.lineBreakMode == NSSLineBreakByWordWrapping) {
            textWidth *= kNSLineBreakWordWrapTextWidthScalingFactor;
        }
...
- (NSArray *)accessibilityElements {
    if (!_accessibilityElements) {
        @synchronized(self) {
            ...
            for (NSAttributedLabelLink *link in self.linkModels) {
                ...
                if (accessibilityLabel) {
                    NSAccessibilityElement *linkElement = [[NSAccessibilityElement alloc] initWithAccessibilityContainer:self];
            ...
            NSAccessibilityElement *baseElement = [[NSAccessibilityElement alloc] initWithAccessibilityContainer:self];
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            NSAttributedLabelLink *link = [self linkAtPoint:touchPoint];
